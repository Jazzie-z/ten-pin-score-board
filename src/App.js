...
import { MAX_ROLLS, GENERAL_ROLL_LIMIT, MAX_ROLLS_PER_FRAME, MAX_SCORE } from "./Constants/constant";
...
function App() {
  ...
  const isNextFrameAvailble = () => {
    ...
    if (rolls.length === (MAX_ROLLS - 1) && rolls[GENERAL_ROLL_LIMIT - 1] + rolls[GENERAL_ROLL_LIMIT] < MAX_SCORE) {
      return false;
    }
    return true;
  };
  ...
  const isSecondRoll = () => (!isFirstRoll() && rolls.length < GENERAL_ROLL_LIMIT);
  const isLastRoll = () => (rolls.length === GENERAL_ROLL_LIMIT && rolls[GENERAL_ROLL_LIMIT - 1] !== MAX_SCORE)
  const isLastBonuspresent = () => (rolls.length === (MAX_ROLLS - 1) && isStrike(rolls[GENERAL_ROLL_LIMIT - 1]) && rolls[GENERAL_ROLL_LIMIT] !== MAX_SCORE)
  ...
  useEffect(() => {
    ...
    if (rolls.length > (GENERAL_ROLL_LIMIT - 1)) {
      sum += rolls.slice(GENERAL_ROLL_LIMIT - 1, MAX_ROLLS).reduce((a, b) => a + b);
      scores.push(sum);
    }
    setScore(scores);
  }, [rolls]);
